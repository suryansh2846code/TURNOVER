...
>
> **On the name:** this document uses **Chitragupta** and `~/Library/Chitragupta`
> throughout. The app was called Lodestone until 2026-09-16; `migration.py` moves
> a pre-rename home eagerly at startup, so a user who has not launched since the
> rename still has their token under the old path. If you are unblocking somebody
> by hand, check which one exists before copying anything into it.
>
...
* **The Cloud project gets rate-limited or suspended for abuse.** Anyone can
  stand up their own consent screen branded "Chitragupta" using the committed
...
| 2 | `~/Library/Chitragupta/google_client_secret.json` | a user we can reach before the next release |
...
3. **Verify it against a real account, without committing anything:**
   ...
   rm -f ~/Library/Chitragupta/google_token.json   # force a fresh consent
   ./.venv/bin/python -c "
...
* The **background scheduler** calls with `interactive=False`, which cannot open
  ...
  Chitragupta and reconnect Google" message, and that lands in the connector row's
...
cp client_secret_NEW.json ~/Library/Chitragupta/google_client_secret.json
rm -f ~/Library/Chitragupta/google_token.json
```
