...
* **A separate process, never this one.** `-I` isolates it from the environment
  and `-S` from site-packages, so the snippet cannot reach Lodestone's own
...
def run_python(code: str) -> ToolResult:
    ...
    with tempfile.TemporaryDirectory(prefix="lodestone-scratch-") as tmp:
        scratch = Path(tmp)
        script = scratch / "snippet.py"
        script.write_text(source, encoding="utf-8")

        # -I: ignore environment and the user's site dir. -S: no site-packages.
        # Between them the snippet cannot import Lodestone, reach the brain, or
