...
`LODESTONE_DEBUG=1` additionally mirrors everything to stdout for whoever is.
...
def _log_dir() -> Path | None:
    """The Lodestone home's log directory, or None if it cannot be written.
...
def configure(force: bool = False) -> None:
    ...
    #
    # It used to be the other way: the root sat at INFO unless `LODESTONE_DEBUG`
    # was set, and a logger drops a record below its own level *before any
    # handler sees it*. `suppressed()` logs at DEBUG. So every swallowed failure
    # — 107 call sites, the entire reason this module exists — was discarded on
    # the machine of every user who had not set an environment variable they
    # have never heard of. The file said nothing precisely when a bug report
    ...
    # log viewer, and noise there trains people to ignore it. `LODESTONE_DEBUG`
    ...
    stream.setLevel(logging.DEBUG if os.environ.get("LODESTONE_DEBUG")
...
def get_logger(name: str) -> logging.Logger:
    """A logger under the `lodestone` root. Pass `__name__`."""
    configure()
