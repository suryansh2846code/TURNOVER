...
* `ImageInput` is the one shape an image has once it is inside Chitragupta —
...
def parse_data_url(url: str, *, name: str = "") -> ImageInput:
    ...
    if not isinstance(url, str) or not url.startswith("data:"):
        raise ImageError("That attachment isn't an image Chitragupta can read.")
    m = _DATA_URL.match(url.strip())
    if not m:
        raise ImageError("That image is in a format Chitragupta can't read.")
