...
and a developer tool, so Chitragupta fetches and manages them itself:

    ~/Library/Chitragupta/agent-binaries/<vendor>/<version>/   the payload
    ~/Library/Chitragupta/bin/<name>                           a stable symlink
...
def install_cli(vendor: str, on_progress: Callable[[str, int], None] | None = None) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix=f"chitragupta-{vendor}-") as tmp:
        tmp_path = Path(tmp)
