...
app = typer.Typer(add_completion=False, help="Chitragupta — local-first AI agent workspace.")
console = Console()
...
def desktop_app(dev: bool = typer.Option(False, "--dev", help="auto-reload the backend when code changes")):
    """Launch Chitragupta as a native desktop window."""
...
def mcp_install():
    """Print the command to connect the Chitragupta brain to Claude Code."""
    import sys

    from .config import get_settings
    py = sys.executable
    home = get_settings().home
    console.print("[bold]Connect Chitragupta to your terminal Claude:[/]\n")
    console.print(
        f'  claude mcp add chitragupta --env CHITRAGUPTA_HOME="{home}" '
        f'-- "{py}" -m chitragupta.mcp_server.server\n'
    )
