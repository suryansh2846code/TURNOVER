...
app = FastAPI(title="Chitragupta", version="0.2.0", lifespan=_lifespan)
...
def run(reload: bool = False) -> None:
    ...
    s = get_settings()
    print(f"\n  ◆ Chitragupta workspace → http://{s.host}:{s.port}")
    ...
    if reload:
        ...
        uvicorn.run("chitragupta.api.app:app", host=s.host, port=s.port, reload=True,
