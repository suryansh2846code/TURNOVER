...
Exposes the local Chitragupta brain (and tasks + web search) to any MCP-capable
...
exact same local brain the Chitragupta workspace agents use, so your terminal
Claude "already knows you" too.

Register with Claude Code:
    claude mcp add chitragupta -- <python> -m chitragupta.mcp_server.server
(or run `chitragupta mcp-install` to print the exact command).
"""
...
mcp = MCPServer("chitragupta")


@mcp.tool()
def search_brain(query: str, limit: int = 8) -> str:
    """Search the user's personal Chitragupta brain (their memories + a knowledge
...
def remember(text: str, title: str | None = None) -> str:
    """Save a durable fact/preference about the user into their Chitragupta brain,
