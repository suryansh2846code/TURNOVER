...
from lodestone.connectors.base import Connector, SyncResult
...
def isolated(monkeypatch):
    ...
    monkeypatch.setattr("lodestone.scheduler._mcp_servers", list)
    monkeypatch.setattr("lodestone.scheduler._custom_apps", list)
    monkeypatch.setattr("lodestone.scheduler.FilesConnector.synced_paths",
    ...
    monkeypatch.setattr("lodestone.core.store.get_store", lambda: _Store())

    class _Brain:
        def enrich_until_done(self, **_):
            return {}

    monkeypatch.setattr("lodestone.brain.get_brain", lambda: _Brain())

    swept: list[int] = []
    monkeypatch.setattr("lodestone.routines.sweep",
                        lambda new_email_count=0: swept.append(new_email_count))
    return swept


def _registry(monkeypatch, *classes) -> None:
    monkeypatch.setattr("lodestone.scheduler.REGISTRY",
...
def test_a_disabled_scheduler_never_starts(no_real_loop, monkeypatch):
    ...
    monkeypatch.setattr("lodestone.scheduler.get_settings", lambda: _Off())
    sched = Scheduler()
...
def test_a_startup_migration_failure_does_not_stop_the_loop(monkeypatch, quiet_loop):
    ...
    monkeypatch.setattr("lodestone.brain.get_brain", broken)
...
def minute_tick(monkeypatch):
    ...
    **sends email and creates calendar events with nobody present**, once a
    ...
    monkeypatch.setattr("lodestone.notify.desktop_notify",
    ...
    monkeypatch.setattr("lodestone.reminders.get_reminders", lambda: reminders)
    monkeypatch.setattr("lodestone.scheduled.get_scheduled", lambda: scheduled)
    monkeypatch.setattr("lodestone.routines.sweep", lambda new_email_count=0: None)
...
def test_a_scheduled_action_runs_and_is_marked_done(minute_tick):
    ...
    mp.setattr("lodestone.actions.run_now",
...
def test_a_broken_reminder_store_does_not_stop_scheduled_actions(minute_tick):
    ...
    scheduled.rows = [{"id": "s1", "type": "send_email", "params": "{}"}]
    mp.setattr("lodestone.actions.run_now", lambda t, p: {"ok": True, "detail": "sent"})
