...
`lodestone/data/google_client.json` is committed on purpose (`DECISIONS.md` →
...
BUNDLED = Path(__file__).resolve().parents[1] / "lodestone" / "data" / "google_client.json"
...
def test_the_scopes_the_rotation_check_expects_are_the_ones_we_request(scope):
    ...
    """
    from lodestone.connectors.google_auth import SCOPES
